  ...
  describe("Occuland Contract Deployment", function () {
    ...
    describe("Access Roles", function () {
      ...
      it("The admin role should equal the deployer", async function () {
        ...
        expect(await contract.hasRole(adminRole, deployer.address)).to.equal(true);
      });

      it("The admin role should not be open to non-deployer: addr1 (minter)", async function () {
        let adminRole = await contract.DEFAULT_ADMIN_ROLE();
        expect(await contract.hasRole(adminRole, addr1.address)).to.equal(false);
      });
      ...
      it("Minter role should equal addr1", async function () {
        ...
        expect(await contract.hasRole(minterRole, addr1.address)).to.equal(true);
      });

      it("Minter role should not equal anyone else but addr1: deployer", async function () {
        let minterRole = await contract.MINTER_ROLE();
        expect(await contract.hasRole(minterRole, deployer.address)).to.equal(false);
      });

      it("Minter role should not equal anyone else but addr1: addr2", async function () {
        let minterRole = await contract.MINTER_ROLE();
        expect(await contract.hasRole(minterRole, addr2.address)).to.equal(false);
      });

      it("Revoking minter access should stop them from minting", async function(){
        let minterRole = await contract.MINTER_ROLE();
        await contract.revokeRole(minterRole, addr1.address);
        await expect(contract.connect(addr1).mint(addr2.address, '1111')).to.be.reverted;
      });
      ...
      it("Grant minter role to multiple addresses, should be allowed to mint", async function(){
        ...
        expect(await contract.hasRole(minterRole, addr1.address)).to.equal(true);
        expect(await contract.hasRole(minterRole, addr3.address)).to.equal(true);
      });
    });

    describe("Minting", function (){
      it("MINTER role is the only one that can mint a token", async function() {
        expect(await contract.connect(addr1).mint(addr1.address, 'testing uri')).to.be.an('object');
        expect(await contract.totalSupply()).to.be.equal(1);
      });
      it("Deployer cannot mint", async function() {
        await expect(contract.connect(deployer).mint(addr1.address, 'testing uri')).to.be.revertedWith('x');
      });
      it("Non-minter role cannot mint", async function() {
        await expect(contract.connect(addr2).mint(addr1.address, 'testing uri')).to.be.revertedWith('x');
      });
      it("Can mint passing parameters", async function(){
        let mint = await contract.connect(addr1).mint(addr2.address, 'testing uri for addr2');
        await mint.wait();
        let bal = await contract.balanceOf(addr2.address);
        bal = (parseInt(bal.toString()) - 1).toString();
        let res = await contract.tokenOfOwnerByIndex(addr2.address, bal);
        
        expect(await contract.tokenURI(res.toString())).to.be.equal('testing uri for addr2');
    ...
    describe('Asset functions', function(){
      it("Existing asset is transferrable by owner", async function(){
        await contract.connect(addr2).transferFrom(addr2.address, addr1.address, '2');
        expect(await contract.balanceOf(addr1.address)).to.be.equal('2');
        expect(await contract.totalSupply()).to.be.equal(2);
      });
      it("Only owned assets can be transferred", async function(){
        await expect(contract.connect(addr3).transferFrom(addr2.address, addr1.address, '2')).to.be.revertedWith('ERC721: transfer caller is not owner nor approved');
      });
      it("Non-existant assets cannot be transferred", async function(){
        await expect(contract.connect(addr2).transferFrom(addr2.address, addr1.address, '5')).to.be.reverted;
      });
      it("Existing asset can be bridged back and burned only by owner", async function(){
        expect(await contract.connect(addr1).bridgeBack('1')).to.be.an("object");
        expect(await contract.totalSupply()).to.be.equal(1);
      });
      it("Existing asset cannot be bridged back and burned by non-owner", async function(){
        await expect(contract.connect(addr2).bridgeBack('2')).to.be.revertedWith('Non-owner no access');
      });
